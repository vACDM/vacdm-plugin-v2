...
ConfigParser::ConfigParser() : m_errorLine(std::numeric_limits<std::uint32_t>::max()),
                               m_errorMessage() {}

bool ConfigParser::errorFound() const
{
  return std::numeric_limits<std::uint32_t>::max() != this->m_errorLine;
}

std::uint32_t ConfigParser::errorLine() const
{
